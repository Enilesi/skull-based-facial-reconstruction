...
OUT_PATH = Path(__file__).resolve().parents[1] / "models" / "sex_classifier_ratios.joblib"
...
def main():
    ...
    cols = ["XCB", "ZYB", "WFB", "OBBL", "NLB"]
    for c in cols:
        df[c] = pd.to_numeric(df[c], errors="coerce")

    df = df.dropna(subset=["XCB", "ZYB"])
    df = df[df["XCB"] > 0]
    df = df[df["ZYB"] > 0]

    df["ZYB_XCB"] = df["ZYB"] / df["XCB"]
    df["WFB_XCB"] = df["WFB"] / df["XCB"]
    df["OBBL_XCB"] = df["OBBL"] / df["XCB"]
    df["NLB_XCB"] = df["NLB"] / df["XCB"]
    df["NLB_ZYB"] = df["NLB"] / df["ZYB"]

    feature_cols = ["ZYB_XCB", "WFB_XCB", "OBBL_XCB", "NLB_XCB", "NLB_ZYB"]
    ...
    joblib.dump({"model": pipe, "feature_cols": feature_cols}, OUT_PATH)
